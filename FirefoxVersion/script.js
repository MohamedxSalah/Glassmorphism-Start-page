...
(function BookmarksPanel() {
    ...
    // The true root node id. Chrome/Chromium uses '0', but Firefox uses a
    // different id (e.g. 'root________') — hardcoding '0' meant
    // chrome.bookmarks.getChildren('0') silently returned nothing on Firefox,
    // which is why the panel showed "no bookmarks found" even though
    // chrome.bookmarks.search()/getTree() (used elsewhere) worked fine.
    // Resolved once via getTree() the first time the panel opens.
    let rootFolderId = '0';

    function resolveRootId(cb) {
        chrome.bookmarks.getTree(function(tree) {
            rootFolderId = (tree && tree[0] && tree[0].id) || '0';
            cb(rootFolderId);
        });
    }

    // Identifies the "Bookmarks Toolbar" root folder across browsers so it
    // can always be pinned to the top of the root listing.
    function isToolbarFolder(node) {
        if (!node) return false;
        if (node.id === 'toolbar_____' || node.id === '1') return true;
        return /toolbar/i.test(node.title || '');
    }
    ...
    function openPanel() {
        ...
        if (navStack.length === 0) {
            resolveRootId(function(realRootId) {
                loadFolder(realRootId);
            });
        }
    }
    ...
    function loadFolder(folderId, folderTitle) {
        ...
        if (folderId === rootFolderId) {
            navStack = [{ id: rootFolderId, title: 'Bookmarks' }];
        } else {
    ...
    function renderBreadcrumb() {
        ...
        navStack.forEach((crumb, idx) => {
            ...
            if (idx < navStack.length - 1) {
                ...
                // The virtual root can't hold items directly, so skip it.
    ...
    function renderItems(items) {
        ...
        // At the root level, always surface every root folder (Bookmarks
        // Toolbar, Bookmarks Menu, Other/Unfiled Bookmarks, Mobile Bookmarks,
        // etc. — whatever the browser provides) and pin the Toolbar folder
        // first, regardless of the order the browser happens to return them in.
        const isAtRoot = navStack.length === 1 && navStack[0].id === rootFolderId;
        if (isAtRoot) {
            folders.sort((a, b) => (isToolbarFolder(b) ? 1 : 0) - (isToolbarFolder(a) ? 1 : 0));
        }
    ...
    function attachBmDragHandlers(el, node) {
        ...
        el.addEventListener('drop', function(e) {
            ...
            chrome.bookmarks.getChildren(current.id, function(children) {
                ...
                // Desired insertion point, expressed in terms of the CURRENT
                // (pre-move) ordering of children.
                let newIndex = isTopHalf ? targetIndex : targetIndex + 1;

                // bookmarks.move(id, {index}) places the item at `index` in
                // the list AFTER it has already been removed from its old
                // spot. If the dragged item is being reordered within this
                // SAME folder and currently sits BEFORE the insertion point,
                // removing it first shifts every later item back by one —
                // so the target index must be decremented by one too, or the
                // item lands one slot further than intended (e.g. dragging
                // bookmark 1 to just under bookmark 2 was actually landing
                // it under bookmark 3).
                if (dragged.parentId === current.id) {
                    const draggedIndex = children.findIndex(c => c.id === dragged.id);
    ...
    bmAddBtn && bmAddBtn.addEventListener('click', function() {
        const current = navStack[navStack.length - 1] || { id: rootFolderId, title: 'Bookmarks' };
        openAddPopup('bookmark', current.id, current.title);
    });
