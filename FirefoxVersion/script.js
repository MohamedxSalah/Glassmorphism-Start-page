...
(async function preloadWallpaper() {
    if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        const result = await browser.storage.local.get(['bgSetting']);
...
async function initWeather() {
    const result = await browser.storage.local.get(['weatherCache']);
...
async function initExtension() {
    ...
    if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        const result = await browser.storage.local.get(['myShortcuts', 'bgSetting', 'blurSetting']);

        let shortcuts = result.myShortcuts;
        if (!shortcuts) {
            shortcuts = defaultShortcuts;
            await browser.storage.local.set({ myShortcuts: shortcuts });
        }
...
async function rearrangeShortcutsInStorage(fromIndex, toIndex) {
    const result = await browser.storage.local.get(['myShortcuts']);
    let shortcuts = result.myShortcuts || [];
...
addShortcutForm.addEventListener('submit', async function(e) {
    e.preventDefault();
...
bgTypeSelect.addEventListener('change', async function() {
    ...
    if (selectedType === 'default') {
        ...
        await browser.storage.local.set({ bgSetting: bgConfig });
...
async function triggerNewUnsplashRotation(intervalValue) {
    ...
    try {
        ...
        // Use a stable, fully-resolved image URL — it will NOT redirect
        ...
        // Fallback: try a different ID if this one doesn't exist
...
saveBgUrlBtn.addEventListener('click', async function() {
...
blurSlider.addEventListener('input', async function() {
    ...
    if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        await browser.storage.local.set({ blurSetting: parseInt(value) });
    }
});

// --- Reset Blur Button Listener ---
resetBlurBtn.addEventListener('click', async function() {
    ...
    if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        await browser.storage.local.set({ blurSetting: defaultBlur });
    }
});
