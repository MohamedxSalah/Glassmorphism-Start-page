...
// Firefox does not expose chrome.search.query; fall back to the form's action (Google).
function initSearchEngine() {
    const searchForm = document.getElementById('search-form');
    if (!searchForm) return;

    // chrome.search.query is Chrome-only and not available in Firefox MV3.
    // The form's default GET action (Google search) is used instead.
    // No additional listener is needed; the form submits naturally.
}
initSearchEngine();
...
(function BookmarksPanel() {
    ...
    // Root folder id — Chrome uses '0', Firefox uses 'root________'.
    // Detected at startup via chrome.bookmarks.getTree().
    let ROOT_ID = '0';

    function detectRootId(cb) {
        chrome.bookmarks.getTree(function(tree) {
            ROOT_ID = (tree && tree[0] && tree[0].id) || '0';
            if (cb) cb();
        });
    }
    ...
    function openPanel() {
        ...
        if (navStack.length === 0) {
            detectRootId(function() {
                loadFolder(ROOT_ID); // Chrome '0', Firefox 'root________'
            });
        }
    }
    ...
    function loadFolder(folderId, folderTitle) {
        ...
        if (folderId === ROOT_ID) {
            navStack = [{ id: ROOT_ID, title: 'Bookmarks' }];
        } else {
    ...
    function refreshCurrentFolder() {
        ...
        chrome.bookmarks.getChildren(current.id, function(children) {
            let items = children || [];
            // At the root level, put the Bookmarks Toolbar folder first
            if (current.id === ROOT_ID) {
                items = items.slice().sort((a, b) => {
                    const aToolbar = a.id === 'toolbar_____' ? 0 : 1;
                    const bToolbar = b.id === 'toolbar_____' ? 0 : 1;
                    return aToolbar - bToolbar;
                });
            }
            renderItems(items);
        });
    ...
    function attachBmDragHandlers(el, node) {
        ...
        el.addEventListener('drop', function(e) {
            ...
            chrome.bookmarks.getChildren(current.id, function(children) {
                ...
                // If the dragged item currently sits BEFORE the target,
                // removing it shifts everything after it left by one —
                // compensate so the drop lands adjacent to the target as expected.
    ...
    bmAddBtn && bmAddBtn.addEventListener('click', function() {
        const current = navStack[navStack.length - 1] || { id: ROOT_ID, title: 'Bookmarks' };
        openAddPopup('bookmark', current.id, current.title);
    });
